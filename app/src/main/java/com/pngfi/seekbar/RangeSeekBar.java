...
/*
        ...
        original = drawableToBitmap(getResources().getDrawable(mThumbResId));

        lineTop = (int) mHintBGHeight + original.getHeight() / 2 - mSeekBarHeight / 2;
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        int bWidth = original.getWidth();
        if (mSeekBarMode == 1) {
            leftSB.onSizeChanged(lineLeft - bWidth / 2, (lineBottom + lineTop) / 2 - bWidth / 2, original, lineWidth);
        } else {
            leftSB.onSizeChanged(lineLeft - bWidth / 2, (lineBottom + lineTop) / 2 - bWidth / 2, original, lineWidth - mThumbSize);
            rightSB.onSizeChanged(lineLeft + mThumbSize - bWidth / 2, (lineBottom + lineTop) / 2 - bWidth / 2, original, lineWidth - mThumbSize);
        }
    }
 ...
 /*   *//**
    ...
    private class SeekBar {
        ...
        protected void onSizeChanged(int left, int top, Bitmap bmp, int lineWidth) {
            this.left = left;
            this.top = top;
            this.right = left + bmp.getWidth();
            this.bottom = top + bmp.getHeight();

            this.bmp = bmp;
            this.lineWidth = lineWidth;
            this.widthSize = bmp.getWidth();
            this.heightSize = widthSize;
        ...
        protected boolean collide(MotionEvent event) {
            ...
            int offset = (int) (lineWidth * currPercent);
            //增大一点触摸范围
            final int extra =dp2px(getContext(),5);
            return x >= left + offset - extra && x <= right + offset + extra && y >= top - extra && y <= bottom + extra;
        }
                ...
                if (currTouch == leftSB) {
                    if (cellsCount > 1) {
                        ...
                        if (x < lineLeft) {
                            ...
                            percent = (x - (currTouch.left + currTouch.widthSize / 2)) * 1f / (currTouch.lineWidth);
                        }
                        ...
                        if (touchLeftCellsValue > currRightCellsValue - reserveCount) {
                            touchLeftCellsValue = currRightCellsValue - reserveCount;
                            percent = touchLeftCellsValue * cellsPercent;
                            currTouch = rightSB;
                        }
                    } else {
                    ...
                    if (cellsCount > 1) {
                        ...
                        if (touchRightCellsValue < currLeftCellsValue + reserveCount) {
                            touchRightCellsValue = currLeftCellsValue + reserveCount;
                            percent = touchRightCellsValue * cellsPercent;
                            currTouch = leftSB;
                        }
                    } else {
